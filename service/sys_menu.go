...
func getMenuTreeMap(authorityId string) (err error, treeMap map[string][]model.SysMenu) {
	...
	err = lib.GGorm.Where("authority_id = ?", authorityId).Order("sort").Preload("Parameters").Find(&allMenus).Error
...
func AddBaseMenu(menu model.SysBaseMenu) (err error) {
	if !errors.Is(lib.GGorm.Where("name = ?", menu.Name).First(&model.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
		err = errors.New("存在重复name，请修改name")
	}
	err = lib.GGorm.Create(&menu).Error
	return err
}
...
func getBaseMenuTreeMap() (err error, treeMap map[string][]model.SysBaseMenu) {
	...
	err = lib.GGorm.Order("sort").Preload("Parameters").Find(&allMenus).Error
	for _, v := range allMenus {
		treeMap[v.ParentId] = append(treeMap[v.ParentId], v)
	}
	return err, treeMap
...
func GetMenuAuthority(info *request.GetAuthorityId) (err error, menus []model.SysMenu) {
	err = lib.GGorm.Where("authority_id = ? ", info.AuthorityId).Order("sort").Find(&menus).Error
