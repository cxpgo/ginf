...
func CreateAuthority(auth model.SysAuthority) (err error, authority model.SysAuthority) {
	...
	if !errors.Is(lib.GGorm.Where("authority_id = ?", auth.AuthorityId).First(&authorityBox).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同角色id"), auth
	}
	err = lib.GGorm.Create(&auth).Error
	return err, auth
}
...
func CopyAuthority(copyInfo response.SysAuthorityCopyResponse) (err error, authority model.SysAuthority) {
	var authorityBox model.SysAuthority
	if !errors.Is(lib.GGorm.Where("authority_id = ?", copyInfo.Authority.AuthorityId).First(&authorityBox).Error, gorm.ErrRecordNotFound) {
	...
	err = lib.GGorm.Create(&copyInfo.Authority).Error
...
func UpdateAuthority(auth model.SysAuthority) (err error, authority model.SysAuthority) {
	err = lib.GGorm.Where("authority_id = ?", auth.AuthorityId).First(&model.SysAuthority{}).Updates(&auth).Error
	return err, auth
}
...
func DeleteAuthority(auth *model.SysAuthority) (err error) {
	if !errors.Is(lib.GGorm.Where("authority_id = ?", auth.AuthorityId).First(&model.SysUser{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("此角色有用户正在使用禁止删除")
	}
	if !errors.Is(lib.GGorm.Where("parent_id = ?", auth.AuthorityId).First(&model.SysAuthority{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("此角色存在子角色不允许删除")
	}
	db := lib.GGorm.Preload("SysBaseMenus").Where("authority_id = ?", auth.AuthorityId).First(auth)
	err = db.Unscoped().Delete(auth).Error
	if len(auth.SysBaseMenus) > 0 {
		err = lib.GGorm.Model(auth).Association("SysBaseMenus").Delete(auth.SysBaseMenus)
...
func GetAuthorityInfo(auth model.SysAuthority) (err error, sa model.SysAuthority) {
	err = lib.GGorm.Preload("DataAuthorityId").Where("authority_id = ?", auth.AuthorityId).First(&sa).Error
	return err, sa
}
...
func SetDataAuthority(auth model.SysAuthority) error {
	...
	lib.GGorm.Preload("DataAuthorityId").First(&s, "authority_id = ?", auth.AuthorityId)
	err := lib.GGorm.Model(&s).Association("DataAuthorityId").Replace(&auth.DataAuthorityId)
	return err
}
...
func SetMenuAuthority(auth *model.SysAuthority) error {
	var s model.SysAuthority
	lib.GGorm.Preload("SysBaseMenus").First(&s, "authority_id = ?", auth.AuthorityId)
	err := lib.GGorm.Model(&s).Association("SysBaseMenus").Replace(&auth.SysBaseMenus)
	return err
}
...
func findChildrenAuthority(authority *model.SysAuthority) (err error) {
	err = lib.GGorm.Preload("DataAuthorityId").Where("parent_id = ?", authority.AuthorityId).Find(&authority.Children).Error
